<!--
  修改点
  1、增加iconClassPosition 属性：默认为right, 可选值为left, 该属性用来控制icon显示的位置
  2、iconClass 默认值为el-icon-arrow-right
  3、multKey 是否支持快捷键（shift或者ctrl多选）
  4、dbclick 双击选中节点
  5、highlightCurrent 为true
  6、新增 iconRightClass  iconLeftClass属性
-->
...
export default {
  ...
  methods: {
    ...
    getRankNodes() {
      return this.store.getRankNodes();
    },
  ...
  created() {
    ...
    this.$on('tree-node-drag-start', (event, treeNode) => {
      ...
      event.target.innerHTML = `<div class="is-draggStart">${
        event.target.innerHTML
      }</div>`;
      // event.target.className = event.target.className + ' is-draggStart';
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      let dropType;
      let prevData = dropInner ? 0.25 : dropNext ? 0.45 : 1;
      let nextData = dropInner ? 0.75 : dropPrev ? 0.55 : 0;
      const prevPercent = dropPrev ? prevData : -1;
      const nextPercent = dropNext ? nextData : 1;
    ...
    this.$on('tree-node-drag-end', (event) => {
      ...
      event.target.innerHTML = `<div class="is-draggEnd">${
        event.target.innerHTML
      }</div>`;

      // event.target.className = event.target.className + ' is-draggEnd';
