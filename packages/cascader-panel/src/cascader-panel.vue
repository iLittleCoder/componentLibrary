...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'GildataDesign/src/utils/util';
...
export default {
  ...
  methods: {
     ...
     * public methods
     */
    calculateMultiCheckedValue(type) {
      this.checkedValue = this.getCheckedNodes(this.leafOnly).map((node) => node.getValueByOption());
      if (type) {
        this.$parent.checkedValueStore = JSON.parse(JSON.stringify(this.checkedValue));
      }
    },
    scrollIntoView() {
    ...
    clearCheckedNodes() {
      ...
      if (multiple) {
        ...
        this.calculateMultiCheckedValue('clear');
      } else {
