...
export default {
  ...
  methods: {
    ...
    handleTimePick(value, visible, first) {
      if (isDate(value)) {
        // ESLINT错误
        let mody = this.value && modifyTime(this.value, value.getHours(), value.getMinutes(), value.getSeconds());
        const newDate = this.value ? mody : modifyWithTimeString(this.getDefaultValue(), this.defaultTime);
        this.date = newDate;
    ...
    handleDatePick(value) {
      if (this.selectionMode === 'day') {
        // ESLINT
        let mody = this.value && modifyDate(this.value, value.getFullYear(), value.getMonth(), value.getDate());
        let newDate = this.value ? mody : modifyWithTimeString(value, this.defaultTime);
    ...
    handleYearPick(year) {
      if (this.selectionMode === 'year') {
        ...
        this.date = modifyDate(this.date, year, month, 1);
        this.currentView = 'quarter';
      } else {
    ...
    shortcutWeekClick(date) {
      const week = this.firstDayOfWeek;
      // 周日为界限，左右偏移的天数，3217654 例如周一就是 -1，目的是调整前两行日期的位置
      const offsetDay = week > 3 ? 7 - week : -week;
      let newDate = new Date(new Date(date).getTime() - 3600 * 1000 * 24 * (new Date(date).getDay() - 1 - offsetDay));
