...
const RANGE_WEEK_FORMATTER = function(value, format) {
  let formatter = TYPE_VALUE_RESOLVER_MAP.week.formatter;
  if (Array.isArray(value) && value.length === 2) {
    const start = value[0];
    const end = value[1];

    if (start && end) {
      return [formatter(start, format), formatter(end, format)];
    }
  }
  return '';
};

const RANGE_QUARTER_FORMATTER = function(value, format) {
  let formatter = TYPE_VALUE_RESOLVER_MAP.quarter.formatter;
  if (Array.isArray(value) && value.length === 2) {
    const start = value[0];
    const end = value[1];
...
const TYPE_VALUE_RESOLVER_MAP = {
  ...
  quarter: {
    formatter(value, format) {
      ...
      const trueDate = new Date(value);
      let date = formatDate(trueDate, format);
      date = date.replace(/Q/, 'Q' + currQuarter);
      return date;
    },
    parser(text, format) {
      // parse as if a normal date
  ...
  quarterrange: {
    formatter: RANGE_QUARTER_FORMATTER,
    parser: RANGE_PARSER
  }
};
const PLACEMENT_MAP = {
...
export default {
  ...
  methods: {
    ...
    mountPicker() {
      ...
      this.picker.showToday = this.showToday; // 是否显示 ‘今天’按钮
