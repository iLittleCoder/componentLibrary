...
:::demo 分别通过`show-border`设置级联选择器是否显示边框, 通过`self-adaption`设置级联选择器的宽度是否自适应。
...
在多选模式下，勾选父节点真正选中的都是叶子节点。启用该功能后，可让父子节点取消关联，选择任意一级选项。
...
| show-border       | 是否展示 input 边框（新增属性）                                                                     | boolean                 | —                     | true      |
| show-footer       | 是否展示底部按钮（新增属性）                                                                        | boolean                 | —                     | false     |
| confirmButtonText | 确定按钮的文本内容（新增属性）                                                                      | string                  | —                     | 确定      |
| cancelButtonText  | 重置按钮的文本内容（新增属性）                                                                      | string                  | —                     | 重置      |
| self-adaption     | 宽度是否自适应（新增属性）                                                                          | boolean                 |
