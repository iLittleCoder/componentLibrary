...
<script>
  export default {
    methods: {
      open(type) {
        this.$confirm('这里是提醒模态弹窗的正文文案。', '通知标题', {
...
如果你完整引入了 GildataDesign，它会为 Vue.prototype 添加如下全局方法：$msgbox, $alert, $confirm 和 $prompt。因此在 Vue instance 中可以采用本页面中的方式调用 `MessageBox`。调用参数为：
...
```javascript
import { MessageBox } from 'GildataDesign';
```
